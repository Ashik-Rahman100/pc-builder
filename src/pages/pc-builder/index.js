...
const PcBuilderPage = ({ products }) => {
  ...
  return (
    <div>
      <h2 className="text-5xl text-accent font-bold text-center my-8 tracking-widest">
        Custom Pc builder
      </h2>
      <div className="grid grid-cols-1 justify-items-center gap-4 mb-4">
        {data?.data.map((category) => (
          <div
            key={category._id}
            className="card w-5/6 md:w-3/4 bg-base-100 shadow-xl"
          >
            <div className="card-body grid grid-cols-2 gap-8">
              ...
              <div>
                <Link
                  href={`/builder/${category?.category}`}
                  className="btn btn-accent tracking-widest"
                >
                  Choose
                </Link>
              </div>
        ...
        <div>
          {Object.keys(selectedComponents).length > 1 && (
            <h2 className="text-2xl mt-8 font-bold">Selected Components:</h2>
          )}
